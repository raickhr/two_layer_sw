...
"""
Compute forward gradients from cell center to east face.
gradx_face[i,j] ≈ (phi[i+1,j] - phi[i,j]) / dx_n2n[i,j]
Periodic in x via `iper`
"""
function k_calc_forward_gradientx!(gradx_on_east_face, phi, dx_n2n, Nx::Int, Ny::Int)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = (blockIdx().y - 1) * blockDim().y + threadIdx().y

    if i <= Nx && j <= Ny
        ip1 = iper(i + 1, Nx)
        gradx_on_east_face[i, j] = (phi[ip1, j] - phi[i, j]) / dx_n2n[i, j]
    end
    return
end

"""
Compute backward gradients from cell center to west face.
gradx_face[i,j] ≈ (phi[i,j] - phi[i-1,j]) / dx_n2n[i-1,j]
Periodic in x via `iper`
"""
function k_calc_backward_gradientx!(gradx_on_west_face, phi, dx_n2n, Nx::Int, Ny::Int)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = (blockIdx().y - 1) * blockDim().y + threadIdx().y

    if i <= Nx && j <= Ny
        im1 = iper(i - 1, Nx)
        gradx_on_west_face[i, j] = (phi[i, j] - phi[im1, j]) / dx_n2n[im1, j]
    end
    return
end

"""
Compute forward gradients from cell center to north face.
grady_face[i,j] ≈ (phi[i,j+1] - phi[i,j]) / dy_n2n[i,j]
clamped in y via `clamp1`
"""
function k_calc_forward_gradienty!(grady_on_north_face, phi, dy_n2n, Nx::Int, Ny::Int)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = (blockIdx().y - 1) * blockDim().y + threadIdx().y

    if i <= Nx && j <= Ny
        jp1 = clamp1(j + 1, Ny)
        grady_on_north_face[i, j] = (phi[i, jp1] - phi[i, j]) / dy_n2n[i, j]
    end
    return
end

"""
Compute backward gradients from cell center to south face.
grady_face[i,j] ≈ (phi[i,j] - phi[i,j-1]) / dy_n2n[i,j-1]
clamped in y via `clamp1`
"""
function k_calc_backward_gradienty!(grady_on_south_face, phi, dy_n2n, Nx::Int, Ny::Int)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = (blockIdx().y - 1) * blockDim().y + threadIdx().y

    if i <= Nx && j <= Ny
        jm1 = clamp1(j - 1, Ny)
        grady_on_south_face[i, j] = (phi[i, j] - phi[i, jm1]) / dy_n2n[i, jm1]
    end
    return
end
...
"""
Compute ∇⋅(ν ∇q) in FV form, where q is a velocity (e.g. u or v).
"""
