...
function build_baroclinic_tanh_jet(
    ...
    if noise_amp != 0
        ...
        k_mag_target = 2 * FT(π) / (10 * Ld)    # wavelength ≈ 5·Ld
...
function initialize_baroclinic_tanh_jet!(
    ...
    # Total thickness (pure internal mode: H constant)
    state.prog.H     .= state.prog1.h .+ state.prog2.h
    state.prog.H_old .= state.prog.H

    # Barotropic transports from layer transports
    state.prog.M .= state.prog1.m .+ state.prog2.m
    state.prog.N .= state.prog1.n .+ state.prog2.n
