...
function write_state!(
    ...
    # =======================================================
    prog = state.prog
    temp = state.temp #any buffer is okay
    ...
    @. Ubt =  temp.temp_var_x8 #M / max(H, ϵ)
    @. Vbt =  temp.temp_var_y8 #N / max(H, ϵ)
    ...
    # @info "before writing max m1 , m2" maximum(abs.(Array(state.prog.m1))) maximum(abs.(Array(state.prog.m2)))
    # @info "before writing max u1 , u2" maximum(abs.(Array(u1))) maximum(abs.(Array(u2)))
